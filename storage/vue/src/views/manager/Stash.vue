<template>
  <div>
    ...
    <el-table :data="tableData" stripe :header-cell-style="{background:'#f4f3f9',color:'#606266'}"  @selection-change="handleSelectionChange">
      ...
      <el-table-column align="center" prop="area" label="仓库区域"></el-table-column>
      <el-table-column align="center" prop="manager" label="仓库管理员"></el-table-column>
    ...
    <el-dialog title="信息" :visible.sync="dialogFormVisible" width="30%" :close-on-click-modal="false">
      <el-form label-width="100px" style="padding-right: 40px" :model="form" :rules="rules" ref="ruleForm">
        ...
        <el-form-item prop="area" label="仓库区域">
          <el-input v-model="form.area" autocomplete="off"></el-input>
        </el-form-item>
        <el-form-item prop="manager" label="仓库管理员">
...
export default {
  ...
  methods: {
    ...
    del(id) {
      this.request.delete("/stash/" + id).then(res => {
        if (res.code === '200') {
          this.$message.success("操作成功")
          this.load()
        } else {
    ...
    delBatch() {
      ...
      this.request.post("/stash/del/batch", ids).then(res => {
        if (res.code === '200') {
          this.$message.success("操作成功")
          this.load()
        } else {
    ...
    exp() {
      window.open($baseUrl + "/stash/export")
    },
